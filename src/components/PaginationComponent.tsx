...
const getPagesSelection = (
    ...
    const currentPageIsLast = amountOnPage != null && isLastPage({
        page: page, limit: limit, amountOnPage: amountOnPage, totalResults: totalResults ?? null
    });
...
const PaginationComponent = (props: IProps) => {
    const limit = () => props.paginationResult?.limit ?? props.paginationQuery.limit;
    const page = () => props.paginationResult?.page ?? props.paginationQuery.page;

    const pagesSelection = () => getPagesSelection(page(), limit(), props.paginationResult?.totalResults, props.paginationResult?.amountOnPage);

    createEffect(() => {
        const paginationResult = props.paginationResult;
        if (paginationResult) {
            props.onUpdate({page: paginationResult.page, limit: paginationResult.limit});
        }
    });

    const setPage = (page: number) => props.onUpdate({page: page, limit: props.paginationQuery.limit});
    const setLimit = (limit: number) => props.onUpdate({page: props.paginationQuery.page, limit: limit});
    ...
    const getMaxPageInput = () => {
        const lastPage = getLastPage(props.paginationResult?.totalResults, limit());
        if (lastPage != null) {
            return lastPage + 1;
        }
        return undefined;
    ...
    return (
        <div>
            <div class="d-inline-flex gap-1">
                <form onSubmit={onSubmit}>
                    <label>
                        ...
                        <input
                            ...
                            onInput={e => setPage(parseInt(e.currentTarget.value, 10) - 1)}
                            type="number"
                            min={1}
                            max={getMaxPageInput()}
                            step={1}
                ...
                <Show when={getLastPage(props.paginationResult?.totalResults, limit()) || page() > 0}>
                ...
                <Show when={
                    ...
                    && props.paginationResult.amountOnPage >= props.paginationResult.limit}
                >
                    <PaginationButton
            ...
            <PaginationResultsSummary paginationResult={props.paginationResult}/>
        </div>
    );
}
